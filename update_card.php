          ...
          // echo $response;
                    ...
                    if($api_status == 'success')
                    {
                      ...
                      $card_type = encrypt($original_string,$private_secret_key);
                        ...
                        $get_cards = "SELECT card_id, customer_id FROM added_cards WHERE card_id = '$card_id' AND customer_id = '$customer_id' AND removed = 0";
                        $query_card = mysqli_query($connection, $get_cards);
                        $check_count = mysqli_num_rows($query_card);

                                if($check_count < 0){
                                      echo '<div class="alert alert-danger">
                                      <b>Ooops!</b> This card is not valid.
                                  </div>';
                                } else {

                                  $date_added = date("Y-m-d h:i:sa", $d);
                                  $sql = "UPDATE added_cards SET card_number = '$card_number', name_on_card = '$ename_on_card', card_month = '$month', card_year = '$year', cvv = '$cvv', issuing_country = '$issuing_country', bin = '$bin', card_type = '$card_type', issuer_info = '$issuer_info' WHERE card_id = '$card_id' AND customer_id = '$customer_id'";
                                  
                                  // use exec() because no results are returned
                                  $conn->exec($sql);
                                  echo '<div class="alert alert-success">
                                  <b>Hurray!</b> Your card is UPDATED, SAFE and SECURED.
                                  </div>';
                                }
